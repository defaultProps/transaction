<template>
  <div id="transaction-checkbox">
    <input type="checkbox" id="checkbox" :value="checked" @change="$emit('transaction-radiochange', $event.target.checked)">
...
#transaction-checkbox {
  ...
  --transaction-checkbox-width: 50px;
  --transaction-checkbox-height: 32px;
  --transaction-fill: #d1d6ee;
  --transaction-fill-active: #275efe;
  input {
    position: absolute;
    display: block;
    width: var(--transaction-checkbox-width);
    height: var(--transaction-checkbox-height);
    padding: 0;
    margin: 0;
    ...
    &:checked {
      & + svg {
        #bg {
          fill: var(--transaction-fill-active);
        }
      }
    }
  }
